        ...
        <Transition>
            <div v-if="TestStarted == true" class="mt-12 w-2/5 flex flex-col items-center p-5 rounded-xl bg-gray-300">
                ...
                <div class="mt-4 grid grid-cols-2 gap-4 w-full">
                    <button v-for="(item, index) in Questions[CurrentQuestion].options" :key="index" @click="SelectOption(item)" class="bg-emerald-600 py-1 rounded-lg font-medium">{{ item.label }}</button>
                </div>
            </div>
            <div v-else-if="TestStarted == false" class="mt-12">
                <button @click="TestStarted = true" class="bg-emerald-600 py-2 p-3 rounded-lg font-medium">Teste Başla!</button>
            </div>
            <div v-else class="mt-12 w-2/5 flex flex-col items-center">
                <div class= "w-full flex flex-col items-center gap-4 bg-gray-300 p-5 rounded-xl">
                    <h1 class="text-3xl font-semibold">Puan: {{ Point }}</h1>
                    <div :style="{
                        backgroundImage: `url(/${Results[SiyahPopoId].image})`,
                    }" class="h-36 w-36 rounded bg-cover"></div>
                    <h1 class="text-2xl font-semibold">{{ Results[SiyahPopoId].name }}</h1>
                    <p>{{ Results[SiyahPopoId].desciption }}</p>
                </div>
                <button @click="Restart" class="mt-8 bg-emerald-600 py-2 p-3 rounded-lg font-medium">Tekrar Dene</button>
            </div>
        </Transition>
    </div>
</template>
